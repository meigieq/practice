...
with st.sidebar:
    ...
    # 모델 선택 메뉴
    selected_model = st.selectbox("LLM 선택", ["gpt-4o", "gpt-4o-mini"], index=0) # index=0 -> 사용자가 별도의 설정을 안 해주면 기본 0번째 값으로 실행됨됨

    # 검색 결과 개수 설정
    search_result_count = st.slider("검색 결과", min_value=1, max_value=10, value=3)

    # include_domains 설정
    st.subheader("검색 도메인 설정") # 헤더 표시
...
if apply_btn:
    ...
    st.session_state["react_agent"] = create_agent_executor(
        ...
        tools=[tool],  # tool을 다양하게 사용하고 싶으면 tools=[tool, tool1, tool2] 이런 식으로 리스트형식으로 채워넣으면 됨
    )
    st.session_state["thread_id"] = random_uuid() # thread_id 가 같으면 멀티턴 대화가 가능하지만, id가 다르면 이전 맥락을 기억하지 못하게 됨
